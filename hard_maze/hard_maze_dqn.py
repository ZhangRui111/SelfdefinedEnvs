...
class DQNBrain(object):
    ...
    def create_network(self):
        ...
        with tf.variable_scope('eval_net'):
            ...
            self.q_eval_net_out = tf.layers.dense(e3, sum(self.n_actions), kernel_initializer=w_initializer,
        ...
        with tf.variable_scope('target_net'):
            ...
            self.q_target_net_out = tf.layers.dense(t3, sum(self.n_actions), kernel_initializer=w_initializer,
    ...
    def choose_action(self, observation, step):
        ...
        if step < REPLY_START_SIZE:
            ...
            if np.random.uniform() < self.epsilon:
                ...
                action[1] = np.argmax(actions_value[0][self.n_actions[0]:sum(self.n_actions)-1])
            else:
                action[0] = np.random.randint(0, self.n_actions[0])
                action[1] = np.random.randint(self.n_actions[0], sum(self.n_actions))
