...
REPLY_START_SIZE = 1000  # same as run_this.REPLY_START_SIZE
...
class DeepQNetwork:
    ...
    def _build_net(self):
        ...
        '''Dueling DQN change the network's architecture'''
        # ------------------ build evaluate_net ------------------
        with tf.variable_scope('eval_net'):
            ...
            eval_A = tf.layers.dense(e1, self.n_actions, None, kernel_initializer=w_initializer,
                                     bias_initializer=b_initializer, name='eval_A')
            self.q_eval_net_out = eval_V + (eval_A - tf.reduce_mean(eval_A, axis=1, keep_dims=True))
        ...
        with tf.variable_scope('target_net'):
            ...
            target_A = tf.layers.dense(t1, self.n_actions, None, kernel_initializer=w_initializer,
                                       bias_initializer=b_initializer, name='target_A')
            self.q_target_net_out = target_V + (target_A - tf.reduce_mean(target_A, axis=1, keep_dims=True))
            # self.q_target_net_out = tf.layers.dense(t1, self.n_actions, kernel_initializer=w_initializer,
            #                                         bias_initializer=b_initializer, name='t2')
    '''Dueling DQN's main change end here'''
    ...
    '''The most important part of DQN'''
